...
func resourceApsaraStackCSServerlessKubernetes() *schema.Resource {
	return &schema.Resource{
		Create: resourceApsaraStackCSServerlessKubernetesCreate,
		Read:   resourceApsaraStackCSServerlessKubernetesRead,
		Delete: resourceApsaraStackCSServerlessKubernetesDelete,
...
func resourceApsaraStackCSServerlessKubernetesCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := invoker.Run(func() error {
		...
		return WrapErrorf(err, DefaultErrorMsg, "apsarastack_cs_serverless_kubernetes", "CreateServerlessKubernetesCluster", DenverdinoApsaraStackgo)
	}
	if debugOn() {
	...
	return resourceApsaraStackCSServerlessKubernetesRead(d, meta)
}

func resourceApsaraStackCSServerlessKubernetesRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	csService := CsService{client}
	invoker := NewInvoker()
...
func resourceApsaraStackCSServerlessKubernetesDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.ApsaraStackClient)
	csService := CsService{client}
	var requestInfo *cs.Client
	invoker := NewInvoker()
