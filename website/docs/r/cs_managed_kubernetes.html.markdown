...
layout: "apsarastack"
page_title: "Apsarastack: apsarastack_cs_managed_kubernetes"
sidebar_current: "docs-apsarastack-resource-cs-managed-kubernetes"
description: |-
  Provides a ApsaraStack resource to manage container managed kubernetes cluster.
---

# apsarastack\_cs\_managed\_kubernetes

This resource will help you to manage a ManagedKubernetes Cluster in Apsarastack Cloud Kubernetes Service. 
...
resource "apsarastack_vswitch" "vswitches" {
  ...
  vpc_id            = var.vpc_id == "" ? join("", apsarastack_vpc.vpc.*.id) : var.vpc_id
...
// According to the vswitch cidr blocks to launch several vswitches
resource "apsarastack_vswitch" "terway_vswitches" {
...
resource "apsarastack_cs_managed_kubernetes" "k8s" {
  count                 = var.k8s_number
  worker_vswitch_ids    = length(var.vswitch_ids) > 0 ? split(",", join(",", var.vswitch_ids)): length(var.vswitch_cidrs) < 1 ? [] : split(",", join(",", apsarastack_vswitch.vswitches.*.id))
  pod_vswitch_ids       = length(var.terway_vswitch_ids) > 0 ? split(",", join(",", var.terway_vswitch_ids)): length(var.terway_vswitch_cirds) < 1 ? [] : split(",", join(",", apsarastack_vswitch.terway_vswitches.*.id))
...
* `image_id` - Custom Image support. Must based on CentOS7 or ApsaraStackLinux2.
...
* `new_nat_gateway` - (Optional) Whether to create a new nat gateway while creating kubernetes cluster. Default to true. Then openapi in Apsarastack Cloud are not all on intranet, So turn this option on is a good choice.
...
```
$ terraform import apsarastack_cs_managed_kubernetes.main cluster-id
```
