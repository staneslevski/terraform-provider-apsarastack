...
resource "apsarastack_cs_kubernetes" "k8s" {
  count                 = 1
  master_vswitch_ids    = length(var.vswitch_ids) > 0 ? split(",", join(",", var.vswitch_ids)): length(var.vswitch_cidrs) < 1 ? [] : split(",", join(",", apsarastack_vswitch.vswitches.*.id))
...
```
$ terraform import apsarastack_cs_kubernetes.main cluster-id
```
